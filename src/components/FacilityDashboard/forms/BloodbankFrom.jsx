...
import { Select, Input, InputNumber, TimePicker, Spin } from 'antd';
...
const BloodBankForm = ({ capabilities, setCapabilities, timeError, validateOperatingHours, loading }) => {

    // Helper functions to parse and format duration strings
    const parseDuration = (durationString) => {
        if (!durationString) return { value: '', unit: '' };
        const match = durationString.match(/^(\d+)\s*(minutes?|hours?)$/i);
        if (match) {
            return {
                value: parseInt(match[1]),
                unit: match[2].toLowerCase().replace(/s$/, '') + 's' // normalize to plural
            };
        }
        return { value: '', unit: '' };
    };
            ...
            loading ? (
                ...
                <>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-2">
                                    <InputNumber
                                        ...
                                        value={parseDuration(capabilities.averageBloodCollectionTime).value}
                                        onChange={(value) => {
                                            const currentUnit = parseDuration(capabilities.averageBloodCollectionTime).unit || 'minutes';
                                            setCapabilities((prev) => ({
                                                ...prev,
                                                averageBloodCollectionTime: formatDuration(value, currentUnit),
                                            }));
                                        }}
                                        min={0}
                                    ...
                                    <Select
                                        placeholder="Unit"
                                        value={parseDuration(capabilities.averageBloodCollectionTime).unit}
                                        onChange={(unit) => {
                                            const currentValue = parseDuration(capabilities.averageBloodCollectionTime).value || 0;
                                            setCapabilities((prev) => ({
                                                ...prev,
                                                averageBloodCollectionTime: formatDuration(currentValue, unit),
                                            }));
                                        }}
                            ...
                            {capabilities.acceptsReturnedBlood === "Yes" && (
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        <InputNumber
                                            placeholder="0"
                                            value={parseDuration(capabilities.returnTimeFrame).value}
                                            onChange={(value) => {
                                                const currentUnit = parseDuration(capabilities.returnTimeFrame).unit || 'minutes';
                                                setCapabilities((prev) => ({
                                                    ...prev,
                                                    returnTimeFrame: formatDuration(value, currentUnit),
                                                }));
                                            }}
                                            min={0}
                                            className='w-20 flex justify-center items-center'
                                        ...
                                        <Select
                                            placeholder="Unit"
                                            value={parseDuration(capabilities.returnTimeFrame).unit}
                                            onChange={(unit) => {
                                                const currentValue = parseDuration(capabilities.returnTimeFrame).value || 0;
                                                setCapabilities((prev) => ({
                                                    ...prev,
                                                    returnTimeFrame: formatDuration(currentValue, unit),
                                                }));
                                            }}
                                            style={{ height: '48px', width: '120px' }}
                                        >
                            ...
                            {capabilities.providesEmergencyDelivery === "Yes" && (
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        <InputNumber
                                            placeholder="Enter duration"
                                            value={parseDuration(capabilities.emergencyResponseTime).value}
                                            onChange={(value) => {
                                                const currentUnit = parseDuration(capabilities.emergencyResponseTime).unit || 'minutes';
                                                setCapabilities((prev) => ({
                                                    ...prev,
                                                    emergencyResponseTime: formatDuration(value, currentUnit),
                                                }));
                                            }}
                                            min={0}
                                            className='w-20 flex justify-center items-center'
                                        ...
                                        <Select
                                            placeholder="Unit"
                                            value={parseDuration(capabilities.emergencyResponseTime).unit}
                                            onChange={(unit) => {
                                                const currentValue = parseDuration(capabilities.emergencyResponseTime).value || 0;
                                                setCapabilities((prev) => ({
                                                    ...prev,
                                                    emergencyResponseTime: formatDuration(currentValue, unit),
                                                }));
                                            }}
                                            style={{ height: '48px', width: '120px' }}
                                        >
                    ...
                    <div className="border-t pt-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-gray-800">
                                What accreditations or certifications does your lab have (e.g., NAFDAC,HEFAMAA, MLSCN, WHO Standards)?
                            </label>
                            <TextArea
