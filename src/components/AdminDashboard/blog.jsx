...
const BlogForm = ({ editingBlog, onSuccess, onCancel }) => {
  ...
  const categories = [
    ...
    'Lifestyle', 'Health', 'Travel', 'Food', 'Fashion', 'Finance'
  ];
  ...
  const handleFileUpload = (event) => {
    ...
    if (file) {
      if (file.size > 2 * 1024 * 1024) {
        setErrors(prev => ({ ...prev, featuredImage: 'File size should be less than 2MB' }));
        return;
      }
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          {formData.featuredImage && (
            <div className="mt-3">
              <img
                src={`${import.meta.env.VITE_APP_BASE_URL}/${formData?.featuredImage}`}
                alt="Featured"
